...
func PostForm(url string, obj string) ([]byte, error) {
	...
	if response.StatusCode != http.StatusOK {
		log.Fatalf("http PostXML error : uri=", url, ", statusCode=", response.StatusCode)
		return ioutil.ReadAll(response.Body)
	}
	return ioutil.ReadAll(response.Body)
}
...
func GbkXML(params map[string]interface{}) (string, error) {
	mv := mxj.Map(params)
	xmlData, err := mv.Xml()
	if err != nil {
		return "", err
	}
	s := `<?xml version="1.0" encoding="GBK" standalone="yes"?>` + string(xmlData)
	// 替换字符串s中的doc为xml
	s = strings.Replace(s, "doc", "xml", -1)
	return s, nil
}
...
func PostXML(uri string, obj map[string]interface{}) ([]byte, error) {
	...
	if response.StatusCode != http.StatusOK {
		log.Fatalf("http PostXML error : uri=", uri, ", statusCode=", response.StatusCode)
		return ioutil.ReadAll(response.Body)
	}
	return ioutil.ReadAll(response.Body)
}
